dim UPto03,UPto02,REto03,REto02,DOWNfrom03,DOWNfrom02,UncheckMember as long

Sub Start()
    
    UncheckMember = 0 '無法確認狀態的會員筆數'
    UPto03 = 0 
    UPto02 = 0 
    REto03 = 0 
    REto02 = 0 
    ...
    worksheets("START").range("E:E").clear
    worksheets("START").range("H:H").clear
    ...
    Dim exceltable()
    Dim DynRepIDarray() As String
    Dim shopID As String
    Dim d, k, t, sheetend
    
   '' for i = 2 to Datarows step 1
    ''    If len(cells(i,5))>2 Then
     ''       cells(i,5).NumberFormatLocal = "@"
      ''      cells(i,5).value = Format(right(cells(i,5).value , 2), "00")
       '' end if
        'if len(cells(i,6))>2 then
         ''   cells(i,6).NumberFormatLocal = "@"
          ''  cells(i,6).value = Format(right(cells(i,6).value , 2), "00")
       '' End If
   '' next i
